...
@section('content')
    ...
    <section class="content-header">
        <h1>Редактировать заказ № {{ $item->id }}
        @if (!$order->status)
        ...
        @else
            <a href="{{ route('blog.admin.orders.change', $item->id) }} /?status=0" class="btn btn-info
            btn-xs">Вернуть на доработку</a>
        @endif
            <a href="" class="btn btn-xs">
                <form action="{{ route('blog.admin.orders.destroy', $item->id) }}" method="POST" id="delform"
                      style="float: none">
                    @method('DELETE')
                    @csrf
                    <button class="btn btn-danger btn-xs delete" type="submit" >Удалить</button>
                </form>
            </a>
        </h1>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="text" value="@if (isset($order->note)) {{ $order->note}}
                                                 @endif"placeholder="@if (!isset($order->note))комментариев нет @endif" name="comment">
                                            </td>
                                        </tr>
                                        </tbody>
                                    </table>
